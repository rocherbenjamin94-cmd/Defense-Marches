...
export function generateDC1Word(data: DC1FormData): Document {
  ...
  children.push(
    createCheckbox(
      data.candidature.type === 'marche_unique',
      'Marché ou accord-cadre unique',
    ),
  );
  children.push(
    createCheckbox(
      data.candidature.type === 'tous_lots',
      'L\'ensemble des lots de la consultation',
    ),
  );
  children.push(
    createCheckbox(
      data.candidature.type === 'lots_specifiques',
      `Les lots suivants : ${data.candidature.lots || ''}`,
    ),
  );
  ...
  children.push(
    createCheckbox(data.candidat.mode === 'seul', 'Le candidat se présente seul'),
  );

  // Groupement checkbox with type
  if (data.candidat.mode === 'groupement' && data.groupement) {
    children.push(
      new Paragraph({
        children: [
          new TextRun({ text: data.candidat.mode === 'groupement' ? '☑' : '☐', size: 22 }),
          new TextRun({ text: ' Le candidat est membre d\'un groupement  ', size: 22 }),
          new TextRun({ text: data.groupement.type === 'conjoint' ? '☑' : '☐', size: 22 }),
          new TextRun({ text: ' Conjoint  ', size: 22 }),
          new TextRun({ text: data.groupement.type === 'solidaire' ? '☑' : '☐', size: 22 }),
          new TextRun({ text: ' Solidaire', size: 22 }),
        ],
    ...
    children.push(
      createCheckbox(false, 'Le candidat est membre d\'un groupement'),
    );
  }
  ...
  children.push(createSubHeader('Justification des capacités'));
  children.push(
    createCheckbox(
      data.engagements.type_capacites === 'dc2',
      'Via le formulaire DC2',
    ),
  );
  children.push(
    createCheckbox(
      data.engagements.type_capacites === 'documents',
