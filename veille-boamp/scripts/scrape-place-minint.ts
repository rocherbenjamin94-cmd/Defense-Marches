#!/usr/bin/env npx tsx
/**
 * Script de scraping PLACE quotidien - Ministère de l'Intérieur
 ...
 *
 * Usage:
 *   npx tsx scripts/scrape-place-minint.ts
 *
...
import { scrapePlaceMinint, MarchePlace, MINISTRY_CONFIG } from '../src/lib/place';

const PLACE_CACHE_KEY = MINISTRY_CONFIG.MININT.cacheKey;
...
async function main() {
    console.log('=== Script de scraping PLACE - Ministère de l\'Intérieur (10 derniers jours) ===');
    ...
    try {
        ...
        console.log(`\n📡 Scraping PLACE (MININT) pour les ${DAYS_TO_SCRAPE} derniers jours...`);
        ...
        for (let i = 0; i < datesToScrape.length; i++) {
            ...
            try {
                const marches = await scrapePlaceMinint(date);
        ...
        console.log(`\n✅ Cache mis à jour:`);
        console.log(`   - Clé: ${PLACE_CACHE_KEY}`);
